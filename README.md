# Portfolio Website

## Project Overview

This portfolio website showcases my personal profile, core skills, featured projects, and a contact form. The main objectives were to create a visually appealing, responsive website using HTML5 and CSS.

## GitHub Repository

- Repository URL: https://github.com/Tadiwanasheedricktembo/week1-portfolio-website

## Setup and Installation

1. Clone or download the repository.
2. Open the `Week 1` folder.
3. Open `index.html` in your browser.

## Code Structure

- `index.html` — Main website content and semantic structure.
- `style.css` — External stylesheet containing layout, colors, typography, responsiveness, and hover interactions.
- `README.md` — Project documentation and technical explanation.
- `images/profile.png` — Profile image displayed on the homepage.

## Styling and Responsiveness

The project uses an external `style.css` file with the following CSS concepts:

- Element selectors: `body`, `header`, `nav`, `button`, `img`
- Class selectors: `.hero`, `.section-card`, `.skills-list`, `.btn`, `.form-group`
- ID selectors: `#about`, `#skills`, `#projects`, `#contact`
- Descendant selectors: `header .container`, `.section-card ul li`, `nav a:hover`
- Pseudo-classes: `:hover`, `:focus-visible`
- Media queries for responsive behavior on smaller screens

Responsive design is implemented with `@media` queries to adjust the layout for tablets and mobile devices. The header and hero sections stack vertically on narrow screens, and buttons expand accordingly.

## How Technical Requirements Were Met

- External `style.css` file created and linked from `index.html`.
- Used 3+ different CSS selectors including element, class, ID, descendant, and pseudo-class selectors.
- Added hover effects for navigation links and buttons.
- Implemented responsive page structure using media queries and flexible grid layout.
- Included semantic HTML sections: About, Skills, Projects, Contact.
- Used form fields with accessible labels and focus styles.

## Folder Structure

```text
Week 1/
├── index.html
├── style.css
├── README.md
└── images/
    └── profile.png
```

## Features

- About section
- Skills section
- Contact form
- Navigation menu
- Semantic HTML structure
- Image support
- Responsive design
